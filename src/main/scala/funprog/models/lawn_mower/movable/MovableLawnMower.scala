...
class MovableLawnMower(limits: GridLimits, lawnMowerInfo: GridPositionalInformation) {
    ...
    def move_forward: MovableLawnMower = {
        ...
        if (newX >= 0 && newX <= this.limits.x && newY >= 0 && newY <= this.limits.y) {
            new MovableLawnMower(limits, new GridPositionalInformation(new Position(newX, newY), lawnMowerInfo.direction))
        } else {
            this
        }
    }
    ...
    def rotate(action: String): MovableLawnMower = {
        ...
        new MovableLawnMower(limits, new GridPositionalInformation(lawnMowerInfo.position, newDirection))
    }

    def lawnMowerToGridPositionalInformation(lawnMower: MovableLawnMower): GridPositionalInformation = {
