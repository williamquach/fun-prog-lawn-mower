...
class LawnMowerTxtConverter {
    def parseFileToDomain(parsedFile: String): Try[LawnMowingContext] = {
        if (parsedFile.isEmpty) {
            ...
            head match {
                case None =>
                    Failure(DonneesIncorectesException("Le fichier ne contient pas de ligne."))
                case Some(head) =>
                    getGridLimits(head)
                        .flatMap(gridLimits => {
                        ...
                        .recoverWith({
                            ...
                            case exception: Exception =>
                                Failure(DonneesIncorectesException("Erreur inattendue : " + exception.getMessage))
                        })
            }
        }
    }
            ...
            case width :: height :: Nil =>
                try {
                    ...
                    case _: NumberFormatException =>
                        Failure(DonneesIncorectesException(
    ...
    private def getLawnMowers(lawnMowersInTextFile: List[String], gridLimits: GridLimits): Try[List[LawnMower]] = {
        if (lawnMowersInTextFile.isEmpty)
            Failure(DonneesIncorectesException("Le fichier ne contient pas de tondeuse."))
            ...
            Failure(DonneesIncorectesException(
                "Les informations et/ou les instructions d'une tondeuse sont incorrectement formatées. " +
                    ...
                    "Exemple : \n'1 2 N\nAADADAGGA'\nElles sont actuellement sous la forme : '" + lawnMowersInTextFile.mkString("\n") + "'"
            ))
        } else {
    ...
    private def parseLawnMower(lawnMowerInText: List[String], gridLimits: GridLimits): Try[LawnMower] = {
        lawnMowerInText match {
            case mowerPosition :: mowerInstructions :: Nil =>
                            ...
                            if (xToInt > gridLimits.x)
                                ...
                                Failure(DonneesIncorectesException(
                                    s"La direction de la tondeuse n'est pas correcte. " +
                                        s"Elle doit être une des valeurs suivantes : ${CardinalDirection.values.mkString(", ")}. " +
                ...
                val mowerInstructionsList = mowerInstructions.split("").toList match {
                    case instructions
                        if instructions.forall(Instruction.values.map(_.toString).contains) => Success(instructions)
                    case _ =>
                        Failure(DonneesIncorectesException(
                    ...
                    case Success(mowerPositionList) =>
                        ...
                        mowerInstructionsList match {
                            ...
                            case Success(mowerInstructionsList) =>
                                Success(new LawnMower(
                                    new GridPositionalInformation(
                                        new Position(mowerXYPose, mowerPositionList(1).toInt),
            ...
            case _ => Failure(DonneesIncorectesException(
                "Les informations et/ou les instructions d'une tondeuse sont incorrectement formatées. " +
                    "Elles doivent être de la forme x y orientation et une suite d'instructions. " +
                    "Exemple : \n'1 2 N\nAADADAGGA'\nL'une d'elle est actuellement sous la forme : '" + lawnMowerInText.mkString("\n") + "'")
            )
        }
    }
}
